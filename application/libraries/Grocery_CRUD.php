...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_datetime_input($field_info,$value)
	{
        ...
        $this->set_css($this->default_theme_path.'/bootstrap/jalali/dist/kamadatepicker.min.css');
        $this->set_js_lib($this->default_theme_path.'/bootstrap/jalali/dist/kamadatepicker.min.js');
        $this->set_js_lib($this->default_theme_path.'/bootstrap/jalali/dist/karma.js');
		...
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$datetime' maxlength='19' id='pdate' />
	...
	{
        $this->set_css($this->default_theme_path.'/bootstrap/jalali/css/persian-datepicker.css');
        $this->set_js_lib($this->default_theme_path.'/bootstrap/jalali/js/persian-datepicker.js');
        $this->set_js_lib($this->default_theme_path.'/bootstrap/jalali/js/pdate.js');



        $input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value='$value' maxlength='19' class='pdate' />
		<a class='datetime-input-clear' tabindex='-1'>".$this->l('form_button_clear')."</a>";
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	protected function state_url($url = '', $is_list_page = false)
	{
		...
		if (!empty($this->crud_url_path)) {
		    ...
		    if( sizeof($ci->uri->segments) > 0 ) {
		      ...
		      if( $method_name == 'Admin' && !in_array( 'Admin' , $state_url_array ) ) //there is a scenario that you don't have the index to your url
		        $state_url_array[$num+1] = 'Admin';
		    }
	...
	protected function get_state_info_from_url()
	{
		...
		if($function_name == 'Admin' && !in_array( 'Admin' ,$ci->uri->segments))
			$segment_position++;
